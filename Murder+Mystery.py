...
def build_frequency_table(corpus):
    ...
    for ele in corpus:
        #print (ele)
        if ele in frequency_table.keys():
            frequency_table[ele] = frequency_table[ele] + 1
        else:
            frequency_table[ele] = 1
...
def ngram_creator(text_list):
    ...
    for i in range(len(text_list) -1): 
        out1 = text_list[i] +  " " + text_list[i+1]
        out.append(out1)
    return out
...
class TextSample:
    ...
    def __repr__(self):
        return "Text from author: {author}  \nwith Average len: {av_l}".format(author=self.author,av_l=self.average_sentence_length)
...
print("Frequency comparison of",myrtle_sample.author,"  :", frequency_comparison(murderer_sample.word_count_frequency, myrtle_sample.word_count_frequency))
print("Frequency comparison of",gregg_sample.author," :", frequency_comparison(murderer_sample.word_count_frequency, gregg_sample.word_count_frequency))
...
#murderer_value = murderer_sample.average_sentence_length
print(murderer_sample.author,"average sentence length       :", murderer_sample.average_sentence_length)
print(lily_sample.author,"average sentence length :", lily_sample.average_sentence_length)
print(myrtle_sample.author,"average sentence length   :", myrtle_sample.average_sentence_length)
print(gregg_sample.author,"average sentence length  :", gregg_sample.average_sentence_length,"\n")

print("percent diff for",lily_sample.author,":",percent_difference(murderer_value, lily_sample.average_sentence_length))
print("percent diff for",myrtle_sample.author,"  :",percent_difference(murderer_value, myrtle_sample.average_sentence_length))
print("percent diff for",gregg_sample.author," :",percent_difference(murderer_value, gregg_sample.average_sentence_length))
...
def find_text_similarity(text_sample1, text_sample2):
    ...
    print(" word count similarity      :", word_count_similarity)
